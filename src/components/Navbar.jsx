    ...
    <div className='w-full h-[80px] z-10 bg-[#202E60] text-white drop-shadow-lg relative'>
      <div className='flex justify-between items-center w-full h-full md:max-w-[1240px] m-auto'>
        ...
        <div className='hidden md:flex items-center space-x-6'>
          ...
          <Link to={`/registration`}><button className='px-6 py-2 bg-[#1A9FDA] rounded-md hover:bg-[#0C7AAE] hover:text-white'>Register</button>
          </Link>
      ...
      <ul className={toggle ? 'absolute bg-white w-full px-8 md:hidden' : 'hidden'}>
        ...
        <div className='flex flex-col my-4'>
          ...
          <Link to={`/registration`}><button className='px-8 py-3'>Register</button></Link>
        </div>
      </ul>
    </div>
  )
